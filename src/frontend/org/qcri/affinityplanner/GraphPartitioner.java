...
public class GraphPartitioner extends AffinityGraph {
    public int repartition (int minLoadPerPartition, int maxLoadPerPartition) throws Exception{
    ...
    public void toPlan(String fileName){
        Plan plan = new Plan(fileName);
        plan.clearPartitions();
        int i = 0;
        for(Set<String> vertices : m_partitionVertices){
            plan.addPartition(i);
            i++;
        }
    }
